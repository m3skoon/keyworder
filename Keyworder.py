...
GUI · Anthropic Claude via Google Vertex AI · 49 keywords · batch processing
"""
...
STOP_WORDS = {
    ...
    "windows","xbox","sony","playstation","adobe","photoshop","instagram",
    "facebook","twitter","youtube","tiktok","snapchat","whatsapp","linkedin",
    "amazon","netflix","spotify","tesla","uber","airbnb","nintendo","fortnite",
    "minecraft","gucci","prada","chanel","rolex","nike","adidas","zara","ikea",
    "starbucks","mcdonalds","coca cola","coca-cola","pepsi","disney","pixar",
    "marvel","lego","barbie","pokemon","bitcoin","ethereum","nasdaq","brent","wti",
}
...
TITLE RULE: No brand names, trademarks, city names, landmark names. Generic descriptive wording only.
KEYWORD RULE: ABSOLUTELY FORBIDDEN — brand names, trademarks, city names, personal names, artist names,
industry benchmarks (Nasdaq, Brent, WTI), and any of:
iphone, macbook, android, google, samsung, adobe, photoshop, instagram, facebook, twitter,
youtube, tiktok, amazon, netflix, spotify, tesla, coca-cola, pepsi, starbucks, mcdonalds,
nike, adidas, gucci, disney, pixar, marvel, lego, pokemon, minecraft, bitcoin, ethereum."""

def make_prompt(names, topic, prepend, append):
    lines = "\n".join(f"  [{i+1}] {n}" for i,n in enumerate(names))
    ctx  = f"\nCollection theme: {topic}" if topic.strip() else ""
    pre  = f"\nAdd after position 5: {prepend}" if prepend.strip() else ""
    app_ = f"\nAdd at the end: {append}"    if append.strip()  else ""
    return f"""Analyze {len(names)} media file(s) for Adobe Stock.{ctx}{pre}{app_}
Files:\n{lines}

RULES:
1. Exactly {KW_COUNT} keywords per file, English only.
2. FIRST 5: highest commercial buyer-intent terms (what buyers type in Adobe Stock search).
3. Prepend keywords after position 5 if given. Append keywords at end if given.
4. Fill rest: subjects, colors, mood, setting, emotions, style, demographics.
5. Single words or 2-word phrases. No duplicates. No punctuation. No brands.
6. Title: max 70 chars, natural English, main subject first.
...
def call_vertex(batch, topic, prepend, append, key_path, log_fn):
    content, names = [], []
    for item in batch:
    ...
    url = (f"https://{REGION}-aiplatform.googleapis.com/v1/projects/{PROJECT_ID}"
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.after(300, self._check_key)
    ...
    def _check_key(self):
        saved = self.cfg.get("key_path","")
        for candidate in [Path(saved) if saved else None, app_dir()/"key.json",
                          Path.home()/"key.json", Path("key.json")]:
            if candidate and candidate.exists():
                self.key_path = candidate
                self.key_lbl.config(text="✓ key.json найден",fg=self.GREEN,cursor="arrow")
                return
        self.key_lbl.config(text="⚠ key.json не найден — нажмите чтобы указать",
    ...
    def _pick_key(self,*_):
        if self.key_lbl.cget("cursor")!="hand2": return
        p=filedialog.askopenfilename(title="Укажите key.json",
            filetypes=[("JSON","*.json"),("All","*")])
        if p and os.path.isfile(p):
            self.key_path=Path(p); self.cfg["key_path"]=p; self._save_cfg()
            self.key_lbl.config(text="✓ key.json найден",fg=self.GREEN,cursor="arrow")
    ...
    def _entry(self,parent,var,readonly=False,width=None):
    ...
    def _build(self):
        ...
        tk.Label(hdr,text="Adobe Stock · Vertex AI · 49 keywords",
                 font=("Segoe UI",9),bg=self.ACC,fg="#ccc8ff").pack(side="left")
        inf=tk.Frame(hdr,bg=self.ACC); inf.pack(side="right",padx=12)
        self.ff_lbl=tk.Label(inf,text="…",font=("Segoe UI",8),bg=self.ACC,fg="#fff")
        self.ff_lbl.pack(anchor="e"); self.ff_lbl.bind("<Button-1>",self._pick_ffmpeg)
        self.key_lbl=tk.Label(inf,text="…",font=("Segoe UI",8),bg=self.ACC,fg="#fff")
        self.key_lbl.pack(anchor="e"); self.key_lbl.bind("<Button-1>",self._pick_key)
        ...
        tk.Label(c4,text="Кастомные ключевые слова",font=("Segoe UI",10,"bold"),
                 bg=self.WHITE,fg=self.FG).pack(anchor="w")
        tk.Label(c4,text="В начало (после топ-5):",font=("Segoe UI",9),
                 bg=self.WHITE,fg=self.FG2).pack(anchor="w",pady=(4,2))
        self._entry(c4,self.v_prepend).pack(fill="x",ipady=6,ipadx=8,pady=(0,8))
        tk.Label(c4,text="В конец:",font=("Segoe UI",9),
                 bg=self.WHITE,fg=self.FG2).pack(anchor="w",pady=(0,2))
    ...
    def _start(self):
        ...
        if not self.key_path or not self.key_path.exists():
            messagebox.showerror("Нет key.json",
                "Положите key.json рядом с Keyworder.exe\n"
                "или нажмите ⚠ key.json в шапке чтобы указать путь."); return

        self.cfg.update({"folder":folder,"topic":self.v_topic.get(),
        ...
        threading.Thread(target=self._run,args=(
            folder,self.v_mode.get(),self.v_topic.get(),self.v_prepend.get(),
            self.v_append.get(),self.v_csvname.get(),self.ffmpeg,self.key_path),
            daemon=True).start()
    ...
    def _process(self,folder,mode,topic,prepend,append,csvname,ffmpeg_bin,key_path):
        fp=Path(folder)
        ...
        for bi,batch in enumerate(batches):
            ...
            res=call_vertex(batch,topic,prepend,append,key_path,
            ...
            if bi<len(batches)-1: time.sleep(0.5)
        ...
        with open(adobe,"w",newline="",encoding="utf-8-sig") as f:
            ...
            for r in results: w.writerow([r["filename"],r["title"],", ".join(r["keywords"]),"",""])
        ...
        with open(ss,"w",newline="",encoding="utf-8-sig") as f:
            ...
            for r in results: w.writerow([r["filename"],r["title"],r["title"],", ".join(r["keywords"]),""])
